define([],function() {	
	var businessParam={
		appPath:getPath("app"),	
		init: function () {	
			businessParam.initData();
			$("#businessParamEditBtn").click(function(){
				businessParam.edit();
            });		
		},
		initData: function () {
    		$.ajax({
    			url:businessParam.appPath+"/businessParam/getParamList.do",
    			type:"post",
    			data:{paramGroup:1},
    			dataType:"json",
    			success:function(result){ 
    				var paramType = $("#businessParamType").val();
    				var navHtml = "";
    				...
    				for(var i = 0; i < list.length; i++){
    					...
    					if((paramType == "" && i == 0) || paramType == o.paramType){
        					navHtml += "<li  class=\"active\"><a href=\"#businessParamTab"+tabIndex+"\" data-toggle=\"tab\">"+o.paramType+"</a></li>";
    					}else{
        					navHtml += "<li><a href=\"#businessParamTab"+tabIndex+"\" data-toggle=\"tab\">"+o.paramType+"</a></li>";
    					}
    					tabIndex++;
    				}
    				$("#businessParamTabs").html(navHtml);
    				var contentHtml = "";
					for(var i = 0; i < tabNames.length; i++){
    					...
    					for(var j = 0; j < list.length; j++){
        					...
        					if(tabName == o.paramType){
        						...
        						itemHtml += value;
        						itemHtml += "<span class=\"glyphicon\" onclick=\"editBusinessParam('"+businessParam.appPath+"','"+o.paramId+"')\" style=\"cursor: pointer;\">修改</span>";
		       					itemHtml += "</label>";
		       					itemHtml += "</div>";
		       					itemHtml += "</div>";
        					}
       					...
       					itemHtml += "</div>";
    					contentHtml += "<div role=\"tabpanel\" class=\"tab-pane "+active+"\" id=\"businessParamTab"+i+"\">"+itemHtml+"</div>"; 
					}
    				$("#businessParamContent").html(contentHtml);
    			}
    		});
        },
        edit: function () {        	
        	var form = $("form[name=businessParamForm]");
...
function editBusinessParam(path, id){
	$.ajax({
		url:path+"/businessParam/getParam.do",
		type:"post",
		data:{paramId:id},
		dataType:"json",
		success:function(result){ 
			var o = result.data;
			$("#businessParamType").val(o.paramType);
			$("#businessParamId").val(o.paramId);
			$("#businessParamValueType").val(o.paramValueType);
			$("#businessParamName").html(o.paramName);
			var valueHtml = "";
			if(o.paramValueMode == "text"){
			    valueHtml = "<input type=\"text\" class=\"form-control val\" name=\"paramValue\" value=\""+o.paramValue+"\">";				
				...
				valueHtml = "<input class=\"datepicker form-control val\" name=\"paramValue\" id=\"businessParamValue\" readonly=\"readonly\" value=\""+o.paramValue+"\"/>";
			}
			$("#businessParamValueDiv").html(valueHtml);
			$("#businessParamModalEdit").modal("show");
