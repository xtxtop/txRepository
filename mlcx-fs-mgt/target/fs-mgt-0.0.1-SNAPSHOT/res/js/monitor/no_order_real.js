...
var carMarks = [];
var Map;
var noOrderAllMarks = new Object();
define([],function() {
			var tempCar;
			var noOrder = {
				appPath : getPath("app"),
...
//								}
				...
				mapInit:function() {
					...
					var removeMarkLabel = new ZoomControlHideMarkLabel();  
					map.addControl(removeMarkLabel);
					var showMarkLabel = new ZoomControlShowMarkLabel();  
					map.addControl(showMarkLabel);
				},
				connect:function (){
				    ...
				    webSocket.onmessage = function(event) {  
				        ...
				        var carMonitorList = [];
				        carMonitorList.push(carMonitorObj);
				        tempCar.push(carMonitorObj);
				        noOrder.addMarker(carMonitorList, Map);
				        var pts = [];
			        	var p2 = new BMap.Point(carMonitorObj.longitude,carMonitorObj.latitude);
			        	pts.push(p2);
			        	var carMark = noOrderAllMarks[carMonitorObj.carNo];
				        noOrder.run(pts,carMonitorObj.carPlateNo,carMark,carMonitorObj.courseAngle);
				    };  
...
//					}
					for(var key in noOrderAllMarks){
						Map.removeOverlay(noOrderAllMarks[key]);
					}
					noOrder.addMarker(carStatusList, Map);
					if($("input[name=carDetailDivOrMarker]").val()==1){
				...
				addMarker : function(carStatusList, map) {
					...
					for (var i = 0; i < carStatusList.length; i++) {
						...
						if(monitor.order!=null){
							...
							if(monitor.realTimeAlarmType==1){
								myIcon = new BMap.Icon(noOrder.appPath+ '/res/img/car/car_red.png', new BMap.Size(32, 48));
...
//								noOrderAllMarks.push(marker);
								noOrderAllMarks[monitor.carNo] = marker;
				...
				drawTrack : function(map,points) {
					...
					car = new BMap.Marker(points[0], {
						icon : new BMap.Icon(noOrder.appPath
								+ '/res/img/car/car_blue.png', new BMap.Size(48,48), {
...
//			    button.addClass("buttonFull");
		 ...
		 // 定义一个控件类，即function
			function ZoomControlShowMarkLabel() {
				// 设置默认停靠位置和偏移量
				this.defaultAnchor = BMAP_ANCHOR_TOP_RIGHT;
				this.defaultOffset = new BMap.Size(80, 10);
			}
		 // 通过JavaScript的prototype属性继承于BMap.Control
			ZoomControlShowMarkLabel.prototype = new BMap.Control();
			ZoomControlShowMarkLabel.prototype.initialize = function(map) {
				// 创建一个DOM元素
				var showMarkLabelBtn = document.createElement("button");
				showMarkLabelBtn.className = "btn";
				showMarkLabelBtn.id="noOrderShowMarkLabelBtn";
				// 添加文字说明
				showMarkLabelBtn.appendChild(document.createTextNode("显示车牌号"));
				// 设置样式
				showMarkLabelBtn.style.cursor = "pointer";
				showMarkLabelBtn.style.border = "1px solid gray";
				showMarkLabelBtn.style.backgroundColor = "white";
				// 绑定事件，
				showMarkLabelBtn.onclick = function(e) {
					conctolCarMark.showMarkLabel(tempCar, noOrderAllMarks);
					$("#noOrderShowMarkLabelBtn").hide();
					$("#noOrdercloseCarMarkBtn").show();
				};
				// 添加DOM元素到地图中
				map.getContainer().appendChild(showMarkLabelBtn);
				// 将DOM元素返回
				return showMarkLabelBtn;
			}
			// 定义一个控件类，即function
			function ZoomControlHideMarkLabel() {
				// 设置默认停靠位置和偏移量
				this.defaultAnchor = BMAP_ANCHOR_TOP_RIGHT;
				this.defaultOffset = new BMap.Size(80, 10);
			}
			// 通过JavaScript的prototype属性继承于BMap.Control
			ZoomControlHideMarkLabel.prototype = new BMap.Control();
			ZoomControlHideMarkLabel.prototype.initialize = function(map) {
				// 创建一个DOM元素
				var button = document.createElement("button");
				button.className = "btn";
				button.id = "noOrdercloseCarMarkBtn";
				// 添加文字说明
				button.appendChild(document.createTextNode("关闭车牌号"));
				// 设置样式
				button.style.cursor = "pointer";
				button.style.border = "1px solid gray";
				button.style.backgroundColor = "white";
				// 绑定事件，
				button.onclick = function(e) {
					conctolCarMark.removeMarkerLabel(noOrderAllMarks);
					$("#noOrdercloseCarMarkBtn").hide();
					$("#noOrderShowMarkLabelBtn").show();
				};
				// 添加DOM元素到地图中
				map.getContainer().appendChild(button);
				// 将DOM元素返回
			...
			var conctolCarMark = {
				showMarkLabel:function(carMonitorList,allMarker){
					for(var i=0;i<carMonitorList.length;i++){
						...
						noOrderAllMarks[carNo].setLabel(label);
					}
				},
				removeMarkerLabel:function(marker){
					for(var key in marker){
						Map.removeOverlay(marker[key].getLabel());
					}
				}
			}
