...
<div class="container-fluid">
	<div class="search-input-wrapper">
		...
		<div class="search-input-content">
			<form class="form-horizontal" name="advertMengLongSearchForm">
				<div class="seach-input-border-box">
					<!-- /.box-tools -->
					<div class="seach-input-details close-content clearfix">
							<div class="seach-input-item pull-left">
								<span>广告名称</span>
								<input type="text" class="form-control" name="advertName" placeholder="请选择活动名称">
							</div>
							<div class="seach-input-item pull-left">
								<span>广告类型</span>
								<select class="form-control" name="type" id="advertList_Type">
								</select>
							</div>
							<div class="seach-input-item pull-left">
								<span>是否可用</span>
					...
					<div class="seacher-button-content">
						...
						<button type="button" class="btn-new-type" id="advertMengLongSearchafter">确定</button>
					</div>
				</div>
			</form>
			<!-- /.box-footer -->
	...
	<div class="row show-table-content">
		<div class="col-xs-12">
			<!--定义操作列按钮模板-->
			<script id="advertMengLongTpl" type="text/x-handlebars-template">
				{{#each func}}
				<button type="button" class="btn-new-type btn-{{this.type}}" onclick="{{this.fn}}">{{this.name}}</button> {{/each}}
			</script>
			<div class="box">
				<div class="box-body">
					<table id="advertMengLongList" class="table table-hover table-bettween-far" width="100%" border="1">
					</table>
				</div>
				<!-- /.box-body -->
			</div>
...
<script type="text/javascript">
	$(function() {
		require.config({
			paths: {
				"advertMengLong": "res/js/ml/advert/advert_list"
			}
		});
		require(["advertMengLong"], function(advertMengLong) {
			advertMengLong.init();
		});
