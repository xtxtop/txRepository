...
<div class="container-fluid">
	<div class="search-input-wrapper">
		...
		<div class="search-input-content">
			<form name="customerFeedbackWorkerSearchForm">
				<div class="seach-input-border-box">
					<div class="seach-input-details close-content clearfix">
						<div class="seach-input-item pull-left">
							<span>调度员</span>
							<input class="form-control" name="memberName" placeholder="请输入调度员名称">
						</div>
						<div class="seach-input-item pull-left">
							<span>手机号</span>
					...
					<div class="seacher-button-content">
						...
						<button type="button" class="btn-new-type" id="customerFeedbackWorkerSearch">确定</button>
					</div>
				</div>
				<!-- /.box-body -->
			</form>
	...
	<div class="row show-table-content">
		<div class="col-xs-12">
			<!--定义操作列按钮模板-->
			<script id="customerFeedbackBtnTplWorker" type="text/x-handlebars-template">
				{{#each func}}
				<button type="button" class="btn-new-type btn-{{this.type}}" onclick="{{this.fn}}">{{this.name}}</button> {{/each}}
			</script>
			<div class="box">
				<div class="box-body">
					<table id="customerFeedbackListWorker" class="table table-hover table-bettween-far" width="100%" border="1">
					</table>
				</div>
				<!-- /.box-body -->
			</div>
...
<script type="text/javascript">
	$(function() {
		require.config({
			paths: {
				"customerFeedbackWorker": "res/js/worker/customerFeedback_list"
			}
		});
