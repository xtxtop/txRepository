	...
	function _d(_e) {
		...
		if(_f.tools) {
			if(typeof _f.tools == "string") {
				...
				var _11 = $("").appendTo(_10);
				var tr = _11.find("tr");
				for(var i = 0; i < _f.tools.length; i++) {
					var td = $("<td></td>").appendTo(tr);
					var _12 = $("<a></a>").appendTo(td);
//					_12[0].onclick = eval(_f.tools[i].handler || function() {});
	...
	function _13(_14, _15) {
		...
		$(".tabs-first>.tabs-inner>.tabs-icon").html("<img src='res/img/00.png'>");
		$('.tabs-first>.tabs-inner').css({'padding-left':'30px','borderLeft':'2px solid #D8D8D8'});
		...
		if(_17.tabPosition == "left" || _17.tabPosition == "right") {
			...
			_18.children("div.tabs-tool:not(.tabs-tool-hidden)").css("display", _17.showHeader ? "block" : "none");
	...
	function _28(_29) {
		...
		$("<div class=\"tabs-header\">" + "<div class=\"tabs-wrap\">" + "<ul class=\"tabs\"></ul>" + "</div>" + "</div>").prependTo(_29);
	...
	function _3c(_3d, _3e, pp) {
		_3e = _3e || {};//待添加得页签信息
		var _3f = $.data(_3d, "tabs");//已经存在得页签信息
		var _40 = _3f.tabs;
		...
		var tab = $("<li class=\"a\">" + "<a class=\"tabs-inner\">" + "<span class=\"tabs-title\"></span>" + "<span class=\"tabs-icon\"></span>" + "</a>" + "</li>");
		...
		if(_53.type == "all" || _53.type == "header") {
			...
			if(_55.header) {
				...
				if(_55.closable) {
					...
					$("<a class=\"tabs-close\">X</a>").appendTo(tab);
				} else {
