...
//							e.setAnimation(BMAP_ANIMATION_DROP);
						...
						_setInfoWin : function(g) {
							...
							if (e != -1) {
								...
								$('.carTrack-info').html(f._opts.landmarkPois[e].html);
								var speed = f._opts.landmarkPois[e].speed;
								if(e==f._opts.landmarkPois.length-1){
									speed = 0;
								}
								renderSpeedCharts({
									eleId:'carTrackSpeedCharts',
									chartsTitle:'行驶速度',
									height:200,
									maxValue:240,
									initStr:'km/h',
									showValue:speed==null?0:speed
								});
	...
	function renderSpeedCharts(configObj){
