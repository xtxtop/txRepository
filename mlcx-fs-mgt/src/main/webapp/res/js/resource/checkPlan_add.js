...
var checkPlanAdd = {
		...
		init : function() {
			...
			//新增提交
			$("#saveCheckPlan").click(function(){
				checkPlanAdd.savecheckPlan();
			});
			//新增页面的关闭
			$("#closeAddCheckPlan").click(function(){
				closeTab("新增巡检计划");
			});
			...
			$(".checkPlan-result-detail").click(function(){
				addTab("巡检结果列表",checkPlanAdd.appPath+$(this).data("url"));
            });
			checkPlanAdd.pageListPark();
			checkPlanAdd.pageListUser();
...
savecheckPlan:function() {
	...
	form.ajaxSubmit({
		url : checkPlanAdd.appPath + "/checkPlan/updateCheckPlan.do",
		type : "post",
...
pageListPark:function () {	
	...
	$('#parkLists').dataTable( {
		...
		"ajax": {
			"type": "POST",
			"url": checkPlanAdd.appPath+'/park/pageListPark.do',
			"data": function ( d ) {
				return $.extend( {}, d, {
			...
			$(".checkPlanPark-batch-del").on("click",function(){
				...
				if(len.length==0){
					...
					if(ids.length>1){
						...
						checkPlanAdd.getSelectedRowPark(ids);
					}
				}
...
pageListUser:function () {	
	...
	$('#checkWorkerLists').dataTable( {	
		...
		"ajax": {
			"type": "POST",
			"url": checkPlanAdd.appPath+'/worker/pageListWorker.do',
			"data": function ( d ) {
				return $.extend( {}, d, {
					"pageNo": (d.start/d.length)+1,
					"pageSize": d.length
...
//			$("#checkWorkerTools").append('');
			...
			$(".sysUserCheck-batch-del").on("click",function(){
				...
				if(ids.length>0){
					 checkPlanAdd.getSelectedRowUser(ids);
				}else{
...
getSelectedRowPark:function(ids){
	$("#parkNos").val(ids);
	$.ajax({
 		url: checkPlanAdd.appPath+"/checkPlan/tocheckPark.do?parkNo="+ids, 
...
getSelectedRowUser:function(ids){
	$("#workerId").val(ids);
    $.ajax({
		url: checkPlanAdd.appPath+"/checkPlan/tocheckUser.do?ids="+ids, 
		success: function(data){
			if(data!=null){
				$("#checkUserModal").modal("hide");
				$("#workerName").val(data);
