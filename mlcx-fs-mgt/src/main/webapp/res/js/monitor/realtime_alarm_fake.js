define([],function() {
			var realtimeAlarm = {
				...
				mapInit:function() {
					...
					globalMap = map;
					map.centerAndZoom(new BMap.Point(114.067549,22.560531),13);// 初始化地图，设置中心点坐标和地图级别
...
//						$(".btn").addClass("buttonFull");
//					}
					var myIcon = new BMap.Icon(realtimeAlarm.appPath+ '/res/img/car/car_green.png', new BMap.Size(32, 32));
					...
					marker = new BMap.Marker(new BMap.Point(114.104344,22.562667), {
						icon : myIcon
					}); // 按照地图点坐标生成标记
					map.addOverlay(marker);
					...
					marker = new BMap.Marker(new BMap.Point(114.046852,22.610451), {
						icon : myIcon
					}); // 按照地图点坐标生成标记
					map.addOverlay(marker);
					//realtimeAlarm.addMarker(carStatusList, map);
