define([],function() {
var pricingRuleCencor = {
		appPath : getPath("app"),
		init : function() {
			
			//关闭计费规则审核编辑页
			$("#closeCencorPricingRule").click(function(){
				closeTab("计费规则审核");
            });
			//计费规则审核提交
			$("#EditCencorPricingRule").click(function(){
				pricingRuleCencor.updatecheckPlan();
			});
		},
 updatecheckPlan:function() {
	var form = $("form[name=pricingRuleCencorForm]");
	form.ajaxSubmit({
		url : pricingRuleCencor.appPath + "/pricingRule/updatePricingRule.do",
		type : "post",
