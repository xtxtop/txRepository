define([],function() {	
var orderDayStrikeBalanceAdd = {
		appPath : getPath("app"),
		init : function() {
			//订单冲账信息提交
			$("#addOrderDayStrikeBalance").click(function(){
				orderDayStrikeBalanceAdd.orderDayStrikeBalanceAdd();
			});
			//订单冲账提交页面关闭
			$("#closeOrderDayStrikeBalanceAdd").click(function(){
				closeTab("日租订单冲账");
			});
		},
		orderDayStrikeBalanceAdd:function() {
			var form = $("form[name=orderDayStrikeBalanceAddForm]");
			form.ajaxSubmit({
				url : orderDayStrikeBalanceAdd.appPath + "/orderStrikeBalance/orderStrikeBalanceAdd.do",
				type : "post",
				success : function(res) {
					var msg = res.msg;
					var code = res.code;
					if (code == "1") {
						bootbox.alert("<img src='res/img/tan.png' style='width: 29px;height: 29px;margin-top: -4px'>&nbsp;&nbsp;" + "日租订单冲账操作成功", function() {
							closeTab("日租订单冲账");
							$("#orderDayList").DataTable().ajax.reload(function(){});
						});
					} else {
						bootbox.alert("<img src='res/img/tan.png' style='width: 29px;height: 29px;margin-top: -4px'>&nbsp;&nbsp;" + "日租订单冲账操作失败！");
					}
				},
...
return orderDayStrikeBalanceAdd
})
